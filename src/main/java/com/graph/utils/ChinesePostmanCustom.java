 ...
 */
public class ChinesePostmanCustom<V, E>
{
     ...
     */
    public GraphPath<V, E> getCPPSolution(Graph<V, E> graph,Integer numberVrtex)
    {
        ...
        if (graph.getType().isUndirected())
            return solveCPPUndirected(graph,numberVrtex);
        else
            return solveCPPDirected(graph,numberVrtex);
     ...
     */
    private GraphPath<V, E> solveCPPUndirected(Graph<V, E> graph,Integer vertexNumber)
    {
        ...
        return replaceShortcutEdges(graph, shortcutEdges,vertexNumber);
    }
     ...
     * @return CPP solution (closed walk)
     */
    private GraphPath<V, E> solveCPPDirected(Graph<V, E> graph,Integer numberVrtex)
    {
        ...
        return replaceShortcutEdges(graph, shortcutEdges,numberVrtex);
    }

    private GraphPath<V, E> replaceShortcutEdges(
            Graph<V, E> inputGraph,
            Map<E, GraphPath<V, E>> shortcutEdges,Integer vertexNumber)
    {
        V startVertex = (V) inputGraph.vertexSet().toArray()[vertexNumber];
        V endVertex = (V) inputGraph.vertexSet().toArray()[vertexNumber];
        List<V> vertexList = new ArrayList<>();
        List<E> edgeList = new ArrayList<>();

        List<V> verticesInPathWithShortcuts = new ArrayList<>();
        verticesInPathWithShortcuts.addAll(inputGraph.vertexSet());
        // at least 2
        // vertices
        List<E> edgesInPathWithShortcuts = new ArrayList<>();
        edgesInPathWithShortcuts.addAll(inputGraph.edgeSet());
